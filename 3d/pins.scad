		...
		translate(position) {
			if (direction == "vertical") {
				color([0.08, 0.08, 0.08, 0.9])
					translate([-pin_base_depth / 2, -base_span / 2, 0])
						cube([pin_base_depth, base_span, pin_base_height]);

				for (i = [0 : pin_count - 1]) {
					pin_y = pin_start + i * pin_spacing;
					color([0.85, 0.68, 0.2, 0.9])
						translate([0, pin_y, pin_base_height])
							cylinder(h = pin_height, d = pin_diameter, $fn = 16);
				}
			} else {
				color([0.08, 0.08, 0.08, 0.9])
					translate([-base_span / 2, -pin_base_depth / 2, 0])
						cube([base_span, pin_base_depth, pin_base_height]);

				for (i = [0 : pin_count - 1]) {
					pin_x = pin_start + i * pin_spacing;
					color([0.85, 0.68, 0.2, 0.9])
						translate([pin_x, pin_base_depth / 2, pin_base_height / 2])
							rotate([-90, 0, 0])
								cylinder(h = pin_height, d = pin_diameter, $fn = 16);
				}
			}
